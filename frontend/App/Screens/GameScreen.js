...
const GameScreen = ({ navigation, route }) => {
  ...
  const claimButton = () => {
    ...
    socket.on('ClaimReward', ({ revealedNumbers }) => {
      ...
      if (allRowsRevealed) {
        console.log("All rows revealed");
        socket.emit('endGame', { roomId, winnerId: user.id }); // Notify other players and end the game
      }
      ...
      for (let num of selectedNumbers) {
        if (revealedNumbers.includes(num)) {
          ...
          Alert.alert("You have selected wrong number");
          break;
        }
      }
    })
  ...
  useEffect(() => {
    socket.on('rowClaimed', ({ rowIndex, userId }) => {
      ...
      socket.on('endGame', ({ winnerId }) => {
        ...
        Alert.alert(`Game ended. Winner: ${winnerId}`);
        navigation.reset({
          index: 0,
          routes: [{ name: 'Home' }],
        });
